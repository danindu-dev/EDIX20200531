...
def client_view(request):
    ...
    return render(request, 'BPMS/manage_view.html',
                  {'user_d':users.objects.get(u_user_id=request.session.get('u_name','ALL')), 'page_title': 'View Clients',
...
def bank_view(request):
    ...
    str_slice = str(mc_id_len) + ":"
    return render(request, 'BPMS/manage_view.html',
                  {'user_d':users.objects.get(u_user_id=request.session.get('u_name','ALL')), 'page_title': 'View Banks',
    ...
    if (request.method == "POST"):
        ...
        if request.POST.get('e', '0') == '1':
            ...
            return render(request, 'BPMS/manage_item.html',
                          {'user_d':users.objects.get(u_user_id=request.session.get('u_name','ALL')),'error_id':"",
            ...
            str_slice = str(mc_id_len) + ":"
            return render(request, 'BPMS/manage_item.html',
                      {'user_d':users.objects.get(u_user_id=request.session.get('u_name','ALL')),'error_id':"", 'page_title': 'Bank Details',
                       'item_data': item_for_view, 'mc_id_len': mc_id_len, 'str_slice': str_slice, 'th_id':'Bank ID',
                   'th_name':'Bank Name', 'th_address':'Bank Address', 'th_tel' : 'Contact Number',
                   'th_other':'Acc Number', 'frm_header':'Bank Details'})

        elif request.POST.get('todo', '0') == 'update' and request.POST.get('item_type', '0') == 'client':
            if len(sub_clients.objects.filter(sub_client_id=user_d.u_mc_id.mc_id + request.POST['item_id'])) == 1 :
                item = sub_clients.objects.get(sub_client_id=user_d.u_mc_id.mc_id + request.POST['item_id'])
            ...
            if len(sub_bank.objects.filter(sub_bank_id=user_d.u_mc_id.mc_id + request.POST['item_id'])) == 1 :
                item = sub_bank.objects.get(sub_bank_id=user_d.u_mc_id.mc_id + request.POST['item_id'])
                ...
                return render(request, 'BPMS/manage_item.html',
                              {'user_d': users.objects.get(u_user_id=request.session.get('u_name', 'ALL')),
                               'error_id': "", 'page_title': 'Add New Bank Acc',
                               'item_data': item_for_view, 'mc_id_len': mc_id_len, 'str_slice': str_slice,
                               'th_id': 'Bank ID',
                               'th_name': 'Bank Name', 'th_address': 'Bank Address', 'th_tel': 'Contact Number',
                               'th_other': 'Acc Number', 'frm_header': 'Bank Details'})
            ...
            str_slice = str(mc_id_len) + ":"
            return render(request, 'BPMS/manage_item.html',
                      {'user_d':users.objects.get(u_user_id=request.session.get('u_name','ALL')),'error_id':"",'page_title':'Add New Bank Acc',
                       'item_data': item_for_view, 'mc_id_len': mc_id_len, 'str_slice': str_slice, 'th_id':'Bank ID',
                   'th_name':'Bank Name', 'th_address':'Bank Address', 'th_tel' : 'Contact Number',
                   'th_other':'Acc Number', 'frm_header':'Bank Details'})
            ...
            if request.POST['item_name'] != '':
                error_id = ""
                temp_id = user_d.u_mc_id.mc_id + request.POST['item_id']
                item = sub_bank()
                item.sub_bank_id = temp_id
                item.sub_bank_name = request.POST['item_name']
                item.sub_bank_address = request.POST['item_address']
                item.sub_bank_tel = request.POST['item_tel']
                item.sub_bank_acc = request.POST['item_other']
                ...
                if len(sub_bank.objects.filter(sub_bank_id=temp_id)) == 0 and request.POST['item_id'].replace(" ", "") != "":
                    item.save()
                    item = sub_bank.objects.get(sub_bank_id=temp_id)
                else:
                    error_id = 'Bank ID already in use.'
                    item.sub_client_id = ''
                ...
                return render(request, 'BPMS/manage_item.html',
                              {'user_d':users.objects.get(u_user_id=request.session.get('u_name','ALL')),
                               'error_id':error_id, 'page_title': 'Bank Details',
                           'item_data': item_for_view, 'mc_id_len': mc_id_len, 'str_slice': str_slice, 'th_id':'Bank ID',
                       'th_name':'Bank Name', 'th_address':'Bank Address', 'th_tel' : 'Contact Number',
                       'th_other':'Acc Number', 'frm_header':'Bank Details'})
            else:
            ...
            if request.POST['item_name'] != '':
                ...
                if len(sub_clients.objects.filter(sub_client_id=temp_id)) == 0 and request.POST['item_id'].replace(" ", "") != "":
                    item.save()
                    item = sub_clients.objects.get(sub_client_id=temp_id)
                else:
                    error_id = 'Client ID already in use.'
                    item.sub_client_id = ''
                ...
                return render(request, 'BPMS/manage_item.html',
                              {'user_d':users.objects.get(u_user_id=request.session.get('u_name','ALL')),
                               'error_id':error_id, 'page_title': 'Client Details',
                               'item_data': item_for_view, 'mc_id_len': mc_id_len, 'str_slice': str_slice,
                               'th_id': 'Client ID',
                               'th_name': 'Client Name', 'th_address': 'Client Address', 'th_tel': 'Contact Number',
                               'th_other': 'Contact Person', 'frm_header': 'Client Details'})
            ...
            str_slice = str(mc_id_len) + ":"
            return render(request, 'BPMS/manage_view.html',
                          {'user_d':users.objects.get(u_user_id=request.session.get('u_name','ALL')), 'page_title': 'View Banks','error_id':"",
                           'item_data': item_for_view, 'mc_id_len': mc_id_len, 'str_slice': str_slice,
                           'th_id': 'Bank ID',
                           'th_name': 'Bank Name', 'th_address': 'Bank Address', 'th_tel': 'Contact Number',
                           'th_other': 'Acc Number', 'e': '2'})
