...
def all_job_s(request):
    ...
    if (request.method == "POST"):
        ...
        th = ['Client', 'Job Count', 'Net Amount', 'Discount', 'TAX Amount', 'Total with TAX', 'Received Amount',
              'Due Amount']
        all_invo_not_date = sub_credit_info.objects.filter(c_mc_id=user_d.u_mc_id).order_by('-c_invoice_num')
        ...
        for client in all_clients:
            ...
            client_temp = [client.sub_client_id[mc_id_len:], len(invos_for_client), "{:,.2f}".format(net_total),
        ...
        tdl = [' ', len(all_invo_oc), "{:,.2f}".format(t_net_total), "{:,.2f}".format(t_deduction),
               "{:,.2f}".format(t_tax_total), "{:,.2f}".format(t_grand_total), "{:,.2f}".format(t_rcvd_amount),
               "{:,.2f}".format(t_grand_total - t_rcvd_amount)]

    return render(request, 'BPMS/report_sum.html',
                  {'rpt_today': rpt_today, 'user_d': users.objects.get(u_user_id=request.session.get('u_name', 'ALL')),
                   'cr': cr, 'report_title': 'Statement of All Invoices', 'tdl': tdl, 'th': th, 'td': td, 'stat': stat,
                   'page_title': 'Reports', 'today': today, 'from_date': from_date})
...
def unsettled_job_s(request):
    ...
    if (request.method == "POST"):
        ...
        all_invo = unstl_all_invo_not_date.filter(
            c_invoice_date__range=[request.POST['report_from_d'], request.POST['report_to_d']])
        all_clients = sub_clients.objects.filter(sub_mc_id=user_d.u_mc_id)
    ...
    return render(request, 'BPMS/report_sum.html',
                  {'rpt_today': rpt_today, 'user_d': users.objects.get(u_user_id=request.session.get('u_name', 'ALL')),
                   'report_title': 'Statement of Unsettled Invoices', 'tdl': tdl, 'th': th, 'td': td, 'stat': stat,
                   'page_title': 'Reports', 'today': today, 'from_date': from_date})
...
def settled_job_s(request):
    ...
    return render(request, 'BPMS/report_sum.html',
                  {'rpt_today': rpt_today, 'user_d': users.objects.get(u_user_id=request.session.get('u_name', 'ALL')),
                   'report_title': 'Statement of Settled Invoices', 'tdl': tdl, 'th': th, 'td': td, 'stat': stat,
                   'page_title': 'Reports', 'today': today, 'from_date': from_date})
...
def all_job_d(request):
    ...
    if (request.method == "POST"):
        ...
        today = request.POST['report_to_d']
        from_date = request.POST['report_from_d']
        th = ['#', 'Date', 'Invoice Number', 'Client', 'Description', 'Amount', 'Deductions', 'Tax', 'Total',
              'Received', 'Due Amout', 'Rcvd Date', 'Chq No', 'Bank', 'Status']
        all_invo_not_date = sub_credit_info.objects.filter(c_mc_id=user_d.u_mc_id).order_by('-c_invoice_num')
        all_invo = all_invo_not_date.filter(
            c_invoice_date__range=[request.POST['report_from_d'], request.POST['report_to_d']])
        ...
        tdl = [' ', ' ', ' ', ' ', ' ', "{:,.2f}".format(t_net_total), "{:,.2f}".format(t_deduction),
               "{:,.2f}".format(t_tax_total), "{:,.2f}".format(t_grand_total), "{:,.2f}".format(t_rcvd_amount),
               "{:,.2f}".format(t_grand_total - t_rcvd_amount), ' ', ' ', ' ', ' ']

    return render(request, 'BPMS/report_details.html',
                  {'rpt_today': rpt_today, 'user_d': users.objects.get(u_user_id=request.session.get('u_name', 'ALL')),
                   'report_title': ' Detail Statement of All Invoices', 'tdl': tdl, 'th': th, 'td': td, 'stat': stat,
                   'page_title': 'Reports', 'today': today, 'from_date': from_date})
...
def unsettled_job_d(request):
    ...
    return render(request, 'BPMS/report_details.html',
                  {'rpt_today': rpt_today, 'user_d': users.objects.get(u_user_id=request.session.get('u_name', 'ALL')),
                   'report_title': 'Detail Statement of Unsettled Invoices', 'tdl': tdl, 'th': th, 'td': td,
                   'stat': stat, 'page_title': 'Reports', 'today': today, 'from_date': from_date})
...
def settled_job_d(request):
    ...
    return render(request, 'BPMS/report_details.html',
                  {'rpt_today': rpt_today, 'user_d': users.objects.get(u_user_id=request.session.get('u_name', 'ALL')),
                   'report_title': 'Detail Statement of Settled Invoices', 'tdl': tdl, 'th': th, 'td': td, 'stat': stat,
                   'page_title': 'Reports', 'today': today, 'from_date': from_date})
...
def cr(request):
    ...
    if (request.method == "POST"):
        ...
        today = request.POST['report_to_d']
        from_date = request.POST['report_from_d']
        th = ['Date', 'Invoice Number', 'Description', 'Net Amount', 'Discount', 'TAX Amount', 'Total with TAX',
              'Received Amount', 'Due Amount']
        all_invo_not_date = sub_credit_info.objects.filter(c_mc_id=user_d.u_mc_id).order_by('-c_invoice_num')
        all_invo = all_invo_not_date.filter(
            c_invoice_date__range=[request.POST['report_from_d'], request.POST['report_to_d']])
        ...
        for client in all_clients:
            ...
            if len(invos_for_client) != 0:
                ...
                for invo_for_client in invos_for_client:
                    ...
                    ctd_temp = [str(invo_for_client.c_invoice_date), str(invo_for_client.c_invoice_num[mc_id_len:]),
                                desc, "{:,.2f}".format(total),
                                "{:,.2f}".format(deduction_i), "{:,.2f}".format(tax),
                                "{:,.2f}".format(float(invo_for_client.c_total_amount)),
                                "{:,.2f}".format(rcvd_amount_i),
                                "{:,.2f}".format(float(invo_for_client.c_total_amount) - rcvd_amount_i)]
                    ctd.append(ctd_temp)
    ...
    return render(request, 'BPMS/report_cr.html',
                  {'rpt_today': rpt_today, 'user_d': users.objects.get(u_user_id=request.session.get('u_name', 'ALL')),
                   'crd': crd, 'report_title': 'Credit Notice', 'tdl': tdl, 'th': th, 'td': td, 'stat': stat,
                   'page_title': 'Reports', 'today': today, 'from_date': from_date})
...
def client_wise_unsettled(request):
            ...
            if len(invos_for_client) != 0:
                ...
                for invo_for_client in invos_for_client:
                    ...
                    ctd_temp = [invo_count, str(invo_for_client.c_invoice_date),
                                ...
                                "{:,.2f}".format(float(invo_for_client.c_total_amount)),
                                "{:,.2f}".format(rcvd_amount_i),
                                "{:,.2f}".format(float(invo_for_client.c_total_amount) - rcvd_amount_i), "OPEN"]
                    ctd.append(ctd_temp)
    ...
    return render(request, 'BPMS/report_cr.html',
                  {'rpt_today': rpt_today, 'user_d': users.objects.get(u_user_id=request.session.get('u_name', 'ALL')),
                   'crd': crd, 'report_title': 'Client-wise - Detailed Report of Unsettled', 'tdl': tdl, 'th': th,
                   'td': td, 'stat': stat, 'page_title': 'Reports', 'today': today, 'from_date': from_date})
...
def client_wise_settled(request):
    ...
    return render(request, 'BPMS/report_cr.html',
                  {'rpt_today': rpt_today, 'user_d': users.objects.get(u_user_id=request.session.get('u_name', 'ALL')),
                   'crd': crd, 'report_title': 'Client-wise - Detailed Report of Settled', 'tdl': tdl, 'th': th,
                   'td': td, 'stat': stat, 'page_title': 'Reports', 'today': today, 'from_date': from_date})
...
def bank_summary(request):
    ...
    return render(request, 'BPMS/report_bank_s.html',
                  {'rpt_today': rpt_today, 'user_d': users.objects.get(u_user_id=request.session.get('u_name', 'ALL')),
                   'report_title': 'Bank Summary Statement', 'tdl': tdl, 'th': th, 'td': td, 'stat': stat,
                   'page_title': 'Reports', 'today': today, 'from_date': from_date})
...
def bank_statement(request):
            ...
            if len(invos_for_client) != 0:
                ...
                for invo_for_client in invos_for_client:
                    ...
                    ctd_temp = [invo_count, str(invo_for_client.c_invoice_date),
                                ...
                                "{:,.2f}".format(rcvd_amount_i),
                                "{:,.2f}".format(float(invo_for_client.c_total_amount) - rcvd_amount_i),
                                invo_for_client.c_status.upper()]
                    ctd.append(ctd_temp)
    ...
    return render(request, 'BPMS/report_bank_s.html',
                  {'rpt_today': rpt_today, 'user_d': users.objects.get(u_user_id=request.session.get('u_name', 'ALL')),
                   'report_title': 'Bank Statement', 'tdl': tdl, 'th': th, 'td': td, 'stat': stat,
                   'page_title': 'Reports', 'today': today, 'from_date': from_date})
...
def temp(request):
    ...
    return render(request, 'BPMS/report_temp.html',
                  {'user_d': users.objects.get(u_user_id=request.session.get('u_name', 'ALL')), 'th': th, 'td': td,
                   'page_title': 'Reports', 'today': str(datetime.now().date()),
